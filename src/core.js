...
export const isValidValue = value => ['', 'false', 'null', 'undefined']
    .every((compare) => value != compare);
...
export const getHandler = (handlers) => {
    if (Array.isArray(handlers)) {
        return valueOut => [valueOut, ...(handlers || [])].reduce(
            (valueIn, handler) => handler(valueIn)
        );
